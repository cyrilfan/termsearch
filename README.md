...
- 窗口顶部有一条隐形拖拽条，按住可以把窗口拖到任意位置；拖拽后的位置在本次运行期间会记住，重启程序恢复默认居中位置
- 术语表 `terms.json` 支持外部编辑后自动热重载，无需重启程序
- 托盘图标：打开术语表文件、重新加载、修改热键、检查更新、开机自启动、退出
- 单实例运行，绿色版单文件 exe，不需要安装
- 启动时自动检查 GitHub Release 是否有新版本，检测到后弹出确认框，确认后自动下载替换重启；只碰 exe 本身，不动 `terms.json`/`config.json`；更新成功会自动清理下载用的临时文件，更新失败（无论是下载阶段还是替换阶段）都会提醒，替换阶段失败时还会保留安装包方便排查
...
多台设备共用同一份术语表时用的同步脚本：把"本机部署文件夹里的 terms.json"和"git 仓库里的 terms.json"双向对齐，支持新增词条、追加多义词、补空释义，也支持同步"编辑已有词条"这个操作（配合程序里的编辑词条快捷键）。

流程：`git pull` 拉最新版 → 以"这台设备上次同步到的版本"为基准，对"本机版本"和"拉取后的仓库版本"做三方合并——只有一边相对基准变了就直接采用那一边（比如只有本机编辑过，就把编辑同步上去；只有仓库变了，就采用仓库的）；两边相对基准都变了、还变得不一样，才是真冲突，这次不自动合并，原样打印到屏幕上交给你手动确认，但不会阻塞其他条目正常同步 → 把结果写回本机部署那份，仓库那份只在内容真的变化时才重写并 `commit` + `push`（避免纯格式差异触发无意义的改动）→ 把这次同步到的版本记成下次同步的基准（存在本地 git config 里，不会被提交推送，只影响这台设备自己）。仓库工作目录如果有未提交的改动，会直接中止、不做任何覆盖。
